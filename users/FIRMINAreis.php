...
<html>
	...
	<head>
		<?php
			...
			if ($notcon == null) {
				$find = $conn->query("SELECT pt,".$_COOKIE['lang']." FROM users WHERE nick='FIRMINAreis'");
				$i = $find->fetch_assoc();
	...
	<body>
		...
		<?php $user = 'FIRMINAreis'; include '../design/userinfo.php'; ?>
		<div id='bio'>
			<?php
				if ($_COOKIE['lang'] == 'pt') {
					echo "
			...
			Maria Firmina dos Reis participou da vida intelectual maranhense: colaborou na imprensa local, publicou livros, participou de antologias, e, além disso, também foi musicista e compositora.[10] A autora era abolicionista: ao ser admitida no magistério, aos 22 anos de idade, sua mãe queria que fosse de palanquim receber a nomeação, mas a autora optou por ir a pé, dizendo a sua mãe: \"Negro não é animal para se andar montado nele.\"[11] Chegou também a escrever um \"Hino da Abolição dos Escravos\"[11]
			...
			<br />
			Fonte: <a href='https://pt.wikipedia.org/wiki/Maria_Firmina_dos_Reis' > Wikipedia </a>
					";
				}
				if ($_COOKIE['lang'] == 'en') {
					echo "
		...
		<?php $auctor='FIRMINAreis'; include '../design/poemfind.php'; ?>
